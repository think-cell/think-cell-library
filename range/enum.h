...
namespace tc {
	...
	constexpr Enum enum_cast(T const& t) noexcept {
		...
		_ASSERTE( IsWellDefinedEnum<Enum>(n) );
...
\
inline SReportStream& operator<<(SReportStream& rs, Enum e) \
{ \
	return rs << "0x" << tc::as_unpadded_uc_hex( tc::underlying_cast(e) ); \
} \
_ASSERT_NOT_ORDERED(Enum)

namespace tc {
	struct enumset_all_set_tag final {};
	struct enumset_all_set_but_one_tag final {};
	...
	private:
		...
		using value_type = typename tc::TInt<enum_count<Enum>::value>::unsigned_;
		value_type m_bitset;
...
#ifdef TC_PRIVATE
		friend SReportStream& operator<<(SReportStream& rs, enumset const& sete) noexcept {
			return rs << "0x" << tc::as_padded_uc_hex( tc::explicit_cast<unsigned long>(sete.m_bitset) );
		}
#endif
	...
	template< typename Enum >
	struct range_value<enumset<Enum> > {
		using type=Enum;
	};
...
#define DEFINE_CONTIGUOUS_ENUM(Enum, enumBegin, enumEnd) \
	...
	Enum ## _helper contiguous_enum_impl(Enum); \
	...
	inline Enum& operator++(Enum& e) noexcept { \
		_ASSERT( e!=tc::contiguous_enum<Enum>::end() ); \
		e = tc::enum_cast<Enum>( tc::underlying_cast(e)+1); \
		return e; \
	} \
	inline Enum& operator--(Enum& e) noexcept { \
		_ASSERT( e!=tc::contiguous_enum<Enum>::begin() ); \
		e = tc::enum_cast<Enum>( tc::underlying_cast(e)-1); \
		return e; \
	} \
