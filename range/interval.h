...
namespace tc {
	...
	namespace interval_adl_barrier {
		template< typename T > struct interval;
	}
	using interval_adl_barrier::interval;

	namespace interval_set_adl_barrier {
	...
	using interval_set_adl_barrier::use_set_impl_tag;
	using interval_set_adl_barrier::use_vector_impl_tag;
	using interval_set_adl_barrier::interval_set;
	...
	namespace interval_adl_barrier {
		...
		private:
			...
			using difference_type = typename std::conditional_t<
				has_minus<T>::value
				,/*?*/difference<T const&> //Evaluation of ::type must be delayed, because it only compiles if the predicate is true
		...
		public:
			...
			interval< tc::decayed_result_of_t< Func&(T) > > transform(Func func) const& MAYTHROW {
				...
				return tc::make_interval(func((*this)[tc::lo]), func((*this)[tc::hi]));
			}
		...
		public:
			...
			tc::common_reference_t<tc::decay_t<T>, T2&&> clamp_exclusive(T2&& t) const& noexcept {
				...
				return t<(*this)[tc::lo]
					? (*this)[tc::lo]
					: tc::min(modified((*this)[tc::hi], --_), std::forward<T2>(t));
			}
...
#pragma warning (suppress : 4552) // '-': operator has no effect; expected operator with side-effect
			...
			bool intersects( interval_set<T, interval<T>, SetOrVectorImpl> const& intvlset ) const& noexcept {
				...
				return (itinterval!=boost::end(intvlset) && intersects(*itinterval)) 
					|| (itinterval!=boost::begin(intvlset) && intersects(*(--itinterval)));
			}

			template<typename S> 
			bool intersects(interval<S> const& intvl) const& noexcept {
				bool const bLhsEndFirst= (*this)[tc::hi]<intvl[tc::hi];
				return (*this)[tc::lo]<intvl[tc::lo]
					? ( bLhsEndFirst ? intvl[tc::lo] < (*this)[tc::hi] : intvl[tc::lo] < intvl[tc::hi] )
					: ( bLhsEndFirst ? (*this)[tc::lo] < (*this)[tc::hi] : (*this)[tc::lo] < intvl[tc::hi] );
			}
			...
			interval<T>& operator|=(interval<T> const& intvl) & noexcept {
				_ASSERT( EmptyInterval()==*this || !inclusive_empty() );
				_ASSERT( EmptyInterval()==intvl || !intvl.inclusive_empty() );
...
#ifdef TC_PRIVATE
		DOSAVESTARTTMPL(interval, ((class)(T)) )
			SAVEMEMBERTAG( (*this)[tc::lo], XMLSTR("begin") )
			SAVEMEMBERTAG( (*this)[tc::hi], XMLSTR("end") )
		DOSAVEEND
		LOADCHILDSTARTBEGINTMPL(interval, ((class)(T)) )
			LOADMEMBERTAG( (*this)[tc::lo], XMLSTR("begin") )
			LOADMEMBERTAG( (*this)[tc::hi], XMLSTR("end") )
		LOADCHILDSTARTEND

		template<typename T> 
		SReportStream& operator<<(SReportStream& rs, interval<T> const& intvl) noexcept {
			return rs << "interval(" << intvl[tc::lo] << ';' << intvl[tc::hi] << ')';
		}
#endif
	}
	using interval_adl_barrier::interval;
	...
	template<typename T>
	struct decay< interval_adl_barrier::interval<T> > {
...
//}
	...
	interval< tc::common_type_t< T1, T2 > > make_interval_sort(T1&& t1, T2&& t2) noexcept {
		...
		return t2 < t1
			? interval< tc::common_type_t< T1, T2 > >(std::forward<T2>(t2), std::forward<T1>(t1))
			: interval< tc::common_type_t< T1, T2 > >(std::forward<T1>(t1), std::forward<T2>(t2));
	}
	...
	auto minmax_interval(Rng&& rng) noexcept {
		auto pairit = std::minmax_element(boost::begin(rng), boost::end(rng));
	...
	namespace interval_set_adl_barrier {
		...
		struct use_set_impl_tag final {};
		struct use_vector_impl_tag final {};
		...
		private:
			using Cont=std::conditional_t<
				...
				std::set< TInterval, less_begin< TInterval > >,
				vector_as_set< TInterval, less_begin< TInterval > >
			>;
			Cont m_cont;
		...
		public:
			...
			const_iterator begin() const& noexcept {
				return boost::begin(m_cont);
			}

			const_iterator end() const& noexcept {
				return boost::end(m_cont);
			}
			...
			interval_set& operator|=(TInterval const& interval) & noexcept {
				if (!interval.empty()) {
					...
					if (itinterval != boost::begin(m_cont)) {
					...
					for (; itinterval != boost::end(m_cont) && !(interval[tc::hi] < (*itinterval)[tc::hi]); ) {
						itinterval = m_cont.erase(itinterval);
					}

					if (itinterval != boost::end(m_cont) && !(interval[tc::hi] < (*itinterval)[tc::lo])) {
			...
			interval_set& operator-=(TInterval const& interval) & noexcept {
				if (!interval.empty()) {
					...
					while (itinterval != boost::end(m_cont) && !(interval[tc::hi] < (*itinterval)[tc::hi])) {
						itinterval = m_cont.erase(itinterval);
					}

					if (itinterval != boost::end(m_cont) && (*itinterval)[tc::lo] < interval[tc::hi]) {
			...
			bool contains(T const& t) const& noexcept {
				...
				if( itinterval == boost::begin(m_cont) ) return false;
				--itinterval;
			...
			interval_set& operator&=(TInterval const& interval) & noexcept {
				...
				m_cont.erase( boost::begin(m_cont), itinterval );
			...
			const_iterator interval_below( T const& t ) const& noexcept {
				...
				_ASSERT( itinterval!=boost::begin(m_cont) ); // there must be interval space at or below t
			...
			const_iterator interval_above( T const& t ) const& noexcept {
				...
				if( itintervalSup==boost::begin(m_cont) ) {
					...
					if( t<(*itintervalInf)[tc::hi] ) {
						...
						_ASSERT( itintervalSup!=boost::end(m_cont) ); // there must be interval space at or above t
						return itintervalSup;
					}
				}
			}
			...
			T const& closest_above( T const& t) const& noexcept {
				...
				if( itinterval != boost::begin(m_cont) && t < (*boost::prior(itinterval))[tc::hi] ) {
					return t;
				} else {
			...
			T const& closest_missing(T const& t, tc::lohi lohi) const& noexcept {
				return tc::lo==lohi ? closest_missing_below(t) : closest_missing_above(t);
			}
			...
			const_iterator closest_interval( T const& t ) const& noexcept {
				...
				if( itintervalSup == boost::begin(m_cont) ) return itintervalSup;

				auto itintervalInf=itintervalSup;
				--itintervalInf;
				if( itintervalSup==boost::end(m_cont) || t-(*itintervalInf)[tc::hi] < (*itintervalSup)[tc::lo]-t ) {
					return itintervalInf;
				} else {
					return itintervalSup;
				}
			...
			T const& closest( T const& t) const& noexcept {
				...
				// below first one
				if( itintervalSup == boost::begin(m_cont) ) return (*itintervalSup)[tc::lo];

				auto itintervalInf=itintervalSup;
				--itintervalInf;
				if( t < (*itintervalInf)[tc::hi]) {
					return t;
				} else if( itintervalSup==boost::end(m_cont) || t-(*itintervalInf)[tc::hi] < (*itintervalSup)[tc::lo]-t ) {
			...
			T strict_closest(T const& t) const& noexcept {
				...
				if( t < tInfEnd ) {
					...
					T tInfLast=tInfEnd;
					--tInfLast;
					if( itintervalSup == boost::end(m_cont) || t - tInfLast < (*itintervalSup)[tc::lo] - t ) {
						return tInfLast;
					} else {
						return (*itintervalSup)[tc::lo];
					}
			...
			bool contains( interval_set<T, TInterval, OtherSetOrVectorImpl> const& intvlset ) const& noexcept {
				...
				auto itintervalA = boost::begin(m_cont);
				auto itintervalB = boost::begin(intvlset.m_cont);
				for(;;) {
					if( (*itintervalA)[tc::hi]<(*itintervalB)[tc::hi] ) {
						++itintervalA;
						if( itintervalA==boost::end(m_cont) ) return false;
					} else {
						if( (*itintervalB)[tc::lo]<(*itintervalA)[tc::lo] ) return false;
						++itintervalB;
						if( itintervalB==boost::end(intvlset.m_cont) ) return true;
					}
				}
			}
			...
			interval_set operator~() const& noexcept {
				...
				if(empty()) {
					...
					if(!intvl.empty()) tc::cont_must_emplace_before( intvlset.m_cont, boost::end(intvlset.m_cont), intvl );
			
					for(; itinterval!=end(); ++itinterval) {
						intvl=TInterval( (*itintervalPrevious)[tc::hi], (*itinterval)[tc::lo] );
						_ASSERT(!intvl.empty());
						tc::cont_must_emplace_before( intvlset.m_cont, boost::end(intvlset.m_cont), intvl );
			...
			interval_set& operator|=( interval_set<T, TInterval, SetOrVectorImpl> const& intvlset) & noexcept {
				...
				for(;;) {
					if( itintervalA==end() ) {
						tc::cont_append<tc::return_void>(cont, tc::drop(intvlset.m_cont, itintervalB));
						break;
					} else if( itintervalB==boost::end(intvlset) ) {
						tc::cont_append<tc::return_void>(cont, tc::drop(m_cont, itintervalA));
						break;
					}
					...
					for(;;) {
						...
						if( (*itintervalA)[tc::hi] < (*itintervalB)[tc::hi] ) {
							...
							++itintervalB;
							if( itintervalB==boost::end(intvlset) ) goto end_from_A;
						}
					}

		end_from_A:
					tc::cont_must_emplace_before( cont, boost::end(cont), tc::make_interval( tIntervalBegin, (*itintervalA)[tc::hi] ) );
					++itintervalA;
					continue;
		end_from_B:
					tc::cont_must_emplace_before( cont, boost::end(cont), tc::make_interval( tIntervalBegin, (*itintervalB)[tc::hi] ) );
					++itintervalB;
					continue;
				}
				m_cont.swap( cont );
			...
			auto for_each_intersecting_interval(interval_set<T, TInterval, OtherSetOrVectorImpl> const& intvlset, Func func) const& MAYTHROW  -> tc::common_type_t<
				decltype(tc::continue_if_not_break(func, *boost::begin(m_cont), *boost::begin(intvlset.m_cont), std::declval<TInterval&>())),
				INTEGRAL_CONSTANT(tc::continue_)
			> {
				if(!empty() && !intvlset.empty()) {
					const_iterator itintervalA = boost::begin(m_cont);
					const_iterator itintervalB = boost::begin(intvlset.m_cont);
					...
					while(itintervalA!=boost::end(m_cont) && itintervalB!=boost::end(intvlset.m_cont)) {
			...
			bool intersects(interval_set<T, TInterval, OtherSetOrVectorImpl> const& intvlset ) const& noexcept {
				...
				for_each_intersecting_interval(intvlset, 
					[&]( TInterval const&, TInterval const&, TInterval const&) noexcept -> bool {
						_ASSERT(!bIntersects);
						bIntersects=true;
						return tc::break_;
					});
			...
			interval_set<T, TInterval, SetOrVectorImpl>& operator&=( interval_set<T, TInterval, OtherSetOrVectorImpl> const& intvlset) & noexcept {
				...
				for_each_intersecting_interval(intvlset, 
					[&]( TInterval const&, TInterval const&, TInterval const& intvl ) noexcept {
						tc::cont_must_emplace_before( contIntersection, boost::end(contIntersection),intvl);
					});
			...
			friend bool operator==(  interval_set<T, TInterval, SetOrVectorImpl> const& lhs, TInterval const& rhs ) noexcept {
				return rhs.empty() ? lhs.empty() : tc::contains_single( lhs.m_cont, rhs );
			}
...
#ifdef TC_PRIVATE
		template<typename T, typename TInterval, typename SetOrVectorImpl>
		SReportStream& operator<<(SReportStream& rs, interval_set<T, TInterval, SetOrVectorImpl> const& intvlset) noexcept {
			rs << "interval_set(" << intvlset.interval_count() << ")(";
			int i = 0;
			for( auto it=boost::begin(intvlset); i < 10 && it!=boost::end(intvlset); ++it ) {
				if( 0 < i ) rs << ", ";
				rs << *it;
				++i;
			}
			if( i < intvlset.interval_count() ) rs << ", ...";
			return rs << ')';
		}
#endif
	}
