...
namespace tc {
	namespace explicit_cast_adl_barrier {
		...
		struct SContainerConversionsHelper {
			...
			std::enable_if_t<
				...
				return tc::make_container<TContainer>(tc::transform(std::forward<Rng>(rng),tc::fn_explicit_cast<typename tc::range_value<TContainer>::type>()));
			}
			...
			std::enable_if_t<
				...
				,TContainer > operator()(Rng&& rng) const& noexcept {
				TContainer cont;
				tc::string_range_converter_adl_barrier::SStringRangeConverter<typename tc::range_value<TContainer>::type,typename tc::range_value< std::remove_reference_t<Rng> >::type>::Append(cont,std::forward<Rng>(rng));
				return cont;
			}
		};
		...
		TTarget InternalConvert( TSource&& src, SFirstConvertToUnderlying ) noexcept {
			return tc::explicit_cast<TTarget>(ConvertToUnderlying(std::forward<TSource>(src)));
		}
	...
	template<typename TTarget, typename TSource>
	TTarget explicit_cast(TSource&& src) noexcept {
		return tc::explicit_cast_adl_barrier::InternalConvert<TTarget>(std::forward<TSource>(src), /*prefer no ConvertToUnderlying*/ tc::explicit_cast_adl_barrier::SDirectConvert());
	}
