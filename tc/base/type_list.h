...
namespace tc {
	...
	namespace type {
		...
		namespace no_adl {
			...
			template<typename... T, typename T0>
			struct push_back<list<T...>, T0> final {
				using type = list<T..., T0>;
			};
		...
		namespace no_adl {
			...
			template<typename... T>
			struct take_first<list<T...>, 0> final {
				using type = list<>;
			};

			template<typename T0, typename... T, std::size_t N> requires (0<N) && (N<=size<list<T0, T...>>::value)
			struct take_first<list<T0, T...>, N> final {
		...
		namespace no_adl {
			...
			template<typename... T>
			struct drop_first<list<T...>, 0> final {
				using type = list<T...>;
			};

			template<typename T0, typename... T, std::size_t N> requires (0<N) && (N<=size<list<T0, T...>>::value)
			struct drop_first<list<T0, T...>, N> final {
		...
		template<typename List, typename T>
		using push_back_if_missing_t = std::conditional_t<any_of<List, curry<std::is_same, T>::template type>::value,
			List,
			push_back_t<List, T>
		>;

		template<typename List>
		using unique_t = accumulate_t<list<>, List, push_back_if_missing_t>;
	}
}
