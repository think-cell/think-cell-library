...
namespace tc {
	template<typename RangeReturn, typename Rng, typename Expr> requires std::is_base_of<x3::parser_base, Expr>::value
	...
	template<typename RangeReturn, typename RngWhere, typename RngWhat, typename Pred> requires (!std::is_base_of<x3::parser_base, RngWhat>::value)
	...
	template<typename Cont, typename T> requires (!std::is_base_of<x3::parser_base, T>::value)
	void remove_inplace(Cont& cont, T const& t) noexcept {
		tc::filter_inplace( cont, [&](auto const& _) noexcept { return !tc::equal_to(_, t); } );
	}

	template<typename Cont, typename Expr> requires std::is_base_of<x3::parser_base, Expr>::value
