		...
		return tc_break_or_continue_sequence(
			(tc::for_each(tc_move_if_owned(rng), [&](auto&& t) MAYTHROW {
				...
				return tc_break_or_continue_sequence(
					...
					(tc::continue_if_not_break(funcElement, tc_move_if_owned(t)))
				);
			}))
	...
	namespace no_adl {
		...
		struct [[nodiscard]] join_framed_adaptor {
			...
			friend auto range_output_t_impl(Self&& self) -> tc::type::unique_t<tc::type::concat_t<
				...
				tc::type::join_t<tc::type::transform_t<tc::range_output_t<decltype(*tc_move_if_owned(self).m_baserng)>, tc::range_output_t>>,
