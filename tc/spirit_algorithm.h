...
namespace tc {
	template<typename RangeReturn, typename RngWhere, typename RngWhat, typename Pred, std::enable_if_t<!std::is_base_of<x3::parser_base, RngWhat>::value>* = nullptr>
	...
	template<typename RangeReturn, typename RngWhere, typename RngWhat, std::enable_if_t<!std::is_base_of<x3::parser_base, RngWhat>::value>* = nullptr>
	[[nodiscard]] decltype(auto) search(RngWhere&& rngWhere, RngWhat const& rngWhat) noexcept {
		return tc::search<RangeReturn>(std::forward<RngWhere>(rngWhere), rngWhat, tc::fn_equal_to_or_parse_match());
	}

	template<typename RangeReturn, typename Rng, typename Expr, std::enable_if_t<std::is_base_of<x3::parser_base, Expr>::value>* = nullptr>
