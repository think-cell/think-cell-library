...
TC_HAS_MEM_FN_XXX_TRAIT_DEF( lower_bound, const&, std::declval<typename T::key_type const&>() ) // pretty good indication that datastructure is tree-like: assume erase(itBegin,itEnd) is cheap and preserves iterators
TC_HAS_MEM_FN_XXX_TRAIT_DEF( assign, &, std::declval<tc::range_value_t<T&> const*>(), std::declval<tc::range_value_t<T&> const*>() )
TC_HAS_MEM_FN_XXX_TRAIT_DEF( size, /*not const&, files might flush*/ )
TC_HAS_MEM_FN_XXX_TRAIT_DEF( empty, /*not const&, files might flush*/ )
