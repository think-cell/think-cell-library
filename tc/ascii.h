...
namespace tc {
	...
	template< typename T >
	[[nodiscard]] bool isasciidigit( T ch ) noexcept {
	...
	template< typename T >
	[[nodiscard]] T toasciiupper( T ch ) noexcept {
		if( isasciilower(ch) ) {
	...
	template< typename T >
	[[nodiscard]] T toasciilower( T ch ) noexcept {
		if( isasciiupper(ch) ) {
			return static_cast<T>( ch+('a'-'A') );
		} else {
			return ch;
