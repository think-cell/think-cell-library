...
namespace tc {
	...
	namespace no_adl {
		...
		public:
			scoped_restorer( T&& var ) noexcept
			:	m_var(tc_move_if_owned(var)),
				m_valSaved(VERIFYINITIALIZED(var)) {}
			scoped_restorer( T&& var, scoped_assign_tag_t ) noexcept
			:	m_var(tc_move_if_owned(var)),
				m_valSaved(tc_move_always(VERIFYINITIALIZED(var))) {}
			~scoped_restorer() {
				m_var=tc_move_always(m_valSaved);
			}
	...
	namespace no_adl {
		...
		:	scoped_restorer<T> {
			template<typename T2>
			explicit scoped_assigner( T&& var, T2&& val ) noexcept
			:	scoped_restorer<T>( tc_move_if_owned(var), scoped_assign_tag )
			{
				this->m_var=std::forward<T2>(val);
			}
			template<typename T2>
			explicit scoped_assigner( scoped_change_tag_t, T&& var, T2&& val ) noexcept
	...
	namespace no_adl {
		...
		:	scoped_restorer<T> {
			...
			explicit scoped_assigner_better( T& var, Val&& val, Better better ) noexcept
