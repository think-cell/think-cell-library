...
#define DEFINE_TMPL_FUNC_WITH_CUSTOMIZATIONS(name) \
	...
	namespace name ## _detail { \
		namespace no_adl { \
			...
			private: \
				...
				template<typename... T> \
				static std::true_type check(tc::void_t<decltype(name ## _impl(std::declval<T>()...))>*); /*unevaluated-only*/ \
			public: \
				static constexpr bool value = std::is_same<std::true_type, decltype(check<Args...>(nullptr))>::value; \
			}; \
			template<typename... Args> \
			...
			private: \
				...
				static std::false_type check(...); /*unevaluated-only*/ \
				template<typename... T> \
				static std::true_type check(tc::void_t<decltype(name ## _impl(name ## _adl::adl_tag, std::declval<T>()...))>*); /*unevaluated-only*/ \
			public: \
				static constexpr bool value = std::is_same<std::true_type, decltype(check<Args...>(nullptr))>::value; \
			}; \
			template<typename... Args> \
			...
			private: \
				...
				static std::false_type check(...); /*unevaluated-only*/ \
				template<typename... T> \
				static std::true_type check(tc::void_t<decltype(name ## _default:: name ## _impl(std::declval<T>()...))>*); /*unevaluated-only*/ \
			public: \
				static constexpr bool value = std::is_same<std::true_type, decltype(check<Args...>(nullptr))>::value; \
			}; \
		} \
	...
	template<typename... Args, \
		std::enable_if_t<name ## _detail::has_adl_ ## name ## _impl<Args...>::value>* = nullptr \
	> \
	...
	template<typename... Args, \
		std::enable_if_t<!name ## _detail::has_adl_ ## name ## _impl<Args...>::value>* = nullptr, \
		std::enable_if_t<name ## _detail::has_adl_tag_ ## name ## _impl<Args...>::value>* = nullptr \
	> \
	...
	template<typename... Args, \
		std::enable_if_t<!name ## _detail::has_adl_ ## name ## _impl<Args...>::value>* = nullptr, \
		std::enable_if_t<!name ## _detail::has_adl_tag_ ## name ## _impl<Args...>::value>* = nullptr, \
		std::enable_if_t<name ## _detail::has_default_ ## name ## _impl<Args...>::value>* = nullptr \
	> \
