...
namespace tc {
	namespace no_adl {
		...
		struct reference_or_value {
			...
			template< typename Rhs, std::enable_if_t<tc::is_safely_constructible<T, Rhs&&>::value>* = nullptr >
		...
		struct TC_EMPTY_BASES empty_value {
			static_assert( std::is_empty<T>::value && std::is_trivial<T>::value );

			constexpr empty_value() = default;
			constexpr empty_value(aggregate_tag_t, T) noexcept {}

			constexpr T best_access() const& noexcept { return T(); }
	...
	using reference_or_value = std::conditional_t<
		std::is_empty<std::remove_cv_t<T>>::value && std::is_trivial<std::remove_cv_t<T>>::value,
