...
UNITTESTDEF( create_range ) {
	...
	TEST_init_hack(std::vector, int, v, {1,2,3,4,5,6,7,8});
	TEST_init_hack(std::list, int, l, {1,2,3,4,5,6,7,8,9});
	TEST_init_hack(std::vector, int, exp_sub_r, {3,4,5});
	...
	auto v_r = make_iterator_range(v); TEST_RANGE_LENGTH(v_r, 8);
	...
	auto str_r = make_iterator_range(str); TEST_RANGE_LENGTH(str_r, 6);
	...
	auto wchar_ir = make_iterator_range( mwchar, mwchar + 8); TEST_RANGE_LENGTH(wchar_ir, 8);
