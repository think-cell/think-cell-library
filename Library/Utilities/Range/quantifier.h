...
bool any_of( Rng && rng ) {
	return accumulate( std::forward<Rng>(rng), false, [](bool& bAccu, bool b) {
...
template< typename Rng >
bool all_of( Rng && rng ) {
	return accumulate( std::forward<Rng>(rng), true, [](bool& bAccu, bool b) {
		return continue_if(( bAccu=bAccu && b ));
	} );
}
...
bool contains( Rng && rng, T const& t ) {
	return any_of( tc::transform( std::forward<Rng>(rng), boost::bind<bool>( fn_equal(), _1, boost::cref(t) ) ) );
}
