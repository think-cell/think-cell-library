...
namespace RANGE_PROPOSAL_NAMESPACE {
	...
	//		}
	//	} fn;
	//	boost::function< void(int) > erased=fn; // erased(x) runs fn(x), discards the return value and returns void
	...
	class function< break_or_continue(A0) > : public boost::function< break_or_continue(A0) > {
		typedef boost::function< break_or_continue(A0) > base_;
	public:
		template< typename Func >
	...
	class function< break_or_continue(A0,A1,A2) > : public boost::function< break_or_continue(A0,A1,A2) > {
		typedef boost::function< break_or_continue(A0,A1,A2) > base_;
	public:
		template< typename Func >
		function( Func && func, typename std::enable_if< std::is_base_of< base_, typename std::decay< Func >::type >::value, unused_arg>::type=unused_arg() )
			: base_( base_cast< base_ >( std::forward<Func>(func) ) )
